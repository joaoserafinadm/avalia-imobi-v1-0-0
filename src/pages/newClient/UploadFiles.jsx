...
import {useAutoScroller, DragDropContext, Draggable, Droppable } from "react-beautiful-dnd";
...
export default function UploadFiles(props) {
    ...
    <div className="col-12">
        <div className="row">
            ...
            {/* Adicione a rolagem ao contêiner externo */}
            <div className="col-12 my-2 d-flex align-items-center mb-5" style={{ overflowX: 'auto' }}>
                <DragDropContext onDragEnd={res => handleOrderChange(res)}  autoScrollerOptions={{ speed: 100, sensitivity: 250 }}>
                    <Droppable droppableId="droppable" direction="horizontal">
                        {(provided) => (
                            // Remove overflow e coloca no container externo
                            <div className="d-flex align-items-bottom" {...provided.droppableProps} ref={provided.innerRef}>
                                        ...
                                        {(provided) => (
                                            <div className="my-3" ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} id={index + 'image'}>
                                                ...
                                                <div className="col-12 mx-3 d-flex justify-content-center align-items-top">
                                                    ...
                                                    <button type="button" className="btn-close ms-1" aria-label="Close" onClick={() => { props.setFiles([...props.files?.slice(0, index), ...props.files?.slice(index + 1)]) }}></button>
                                                </div>
                                                <div className="col-12 d-flex justify-content-center">
                                                    {index === 0 && (
                                                        <span className="small fw-bold">Foto de capa</span>
                                                    )}
                                                </div>
                                            </div>
                                        )}
